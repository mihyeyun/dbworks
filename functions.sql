...
-- 함수이름(매개변수)
-- 숫자 관련 함수
-- dual은 오라클에서 사용하는 가상테이블임

SELECT ABS(-10) FROM dual;

-- 3.876를 소수 첫째자리까지 반올림한 값을 구하시오
...
-- 수동 타입 변환
-- 숫자 형식 변환
SELECT TO_NUMBER('250.3') FROM dual;
...
SELECT TO_CHAR(SYSDATE, 'HH24:MI:SS') 시간_24,
       ...
       TO_CHAR(SYSDATE, 'YYYY-MM-DD HH24:MI:SS') 날짜와시간
FROM dual;

-- NVL(칼럼, 치환값)함수로 NULL 처리하기
SELECT name, address, NVL(phone, '000-9000-111') phone
